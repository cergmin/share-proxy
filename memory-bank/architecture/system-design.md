...
- **Публикация контейнера Proxy:** после успешных проверок на `push` в `main` workflow публикует Docker-образ `apps/proxy/Dockerfile` в GHCR (`ghcr.io/<owner>/<repo>-proxy`) с тегами `main` и short SHA. Пуш в Docker Hub сохраняется как опциональный путь и выполняется при наличии секретов `DOCKERHUB_USERNAME` и `DOCKERHUB_TOKEN`.
